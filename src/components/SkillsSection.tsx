      ...
      <div className="max-w-container-max-width mx-auto px-margin-safe">
        ...
        <div className="flex flex-wrap justify-center gap-6">
          {SKILLS_DATA.map((category, index) => (
            <ScrollReveal key={index} className="glass-card p-4 md:p-8 rounded-xl w-full md:w-[calc((100%-3rem)/3)]">
              <div className="flex items-center gap-3 mb-4">
                <span className="material-symbols-outlined text-primary text-3xl">{category.icon}</span>
                <h3 className="font-headline-md text-xl font-semibold md:text-headline-md">{category.title}</h3>
              </div>
